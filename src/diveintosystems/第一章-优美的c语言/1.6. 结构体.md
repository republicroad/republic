...
Arrays and structs are the two ways in which C supports creating collections of data elements. Arrays are used to create an ordered collection of data elements of the same type, whereas **structs** are used to create a collection of data elements of _different types_. A C programmer can combine array and struct building blocks in many different ways to create more complex data types and structures. This section introduces structs, and in the next chapter we [characterize structs in more detail](https://diveintosystems.org/book/C2-C_depth/structs.html#_c_structs) and [show how you can combine them with arrays](https://diveintosystems.org/book/C2-C_depth/structs.html#_arrays_of_structs).
...
### [](https://diveintosystems.org/book/C1-C_intro/structs.html#_defining_a_struct_type)1.6.1. Defining a Struct Type
...
This struct definition adds a new type to C’s type system, and the type’s name is `struct studentT`. This struct defines four fields, and each field definition includes the type and name of the field. Note that in this example, the `name` field’s type is a character array, for [use as a string](https://diveintosystems.org/book/C1-C_intro/arrays_strings.html#_introduction_to_strings_and_the_c_string_library).

### [](https://diveintosystems.org/book/C1-C_intro/structs.html#_declaring_variables_of_struct_types)1.6.2. Declaring Variables of Struct Types
...
### [](https://diveintosystems.org/book/C1-C_intro/structs.html#_accessing_field_values)1.6.3. Accessing Field Values
...
When accessing structs and their fields, carefully consider the types of the variables you’re using. Novice C programmers often introduce bugs into their programs by failing to account for the types of struct fields. [Table 1](https://diveintosystems.org/book/C1-C_intro/structs.html#TabStructTypes) shows the types of several expressions surrounding our `struct studentT` type.
...
[Figure 2](https://diveintosystems.org/book/C1-C_intro/structs.html#FigStructAssign) shows the values of the two student variables after the assignment statement and call to `strcpy` have executed. Note that the figure depicts the `name` fields as the string values they contain rather than the full array of 64 characters.
...
Here’s a [full example program](https://diveintosystems.org/book/C1-C_intro/_attachments/studentTstruct.c) that defines and demonstrates the use of our `struct studentT` type:
...
### [](https://diveintosystems.org/book/C1-C_intro/structs.html#_passing_structs_to_functions)1.6.4. Passing Structs to Functions
...
Here’s a [full example program](https://diveintosystems.org/book/C1-C_intro/_attachments/structfunc.c) using the `checkID` function that takes a struct parameter:
...
The output shows that when `checkID` prints the `age` field, it reflects the function’s change to the `age` field of the parameter `s`. However, after the function call returns, `main` prints the `age` field of `student` with the same value it had prior to the `checkID` call. [Figure 3](https://diveintosystems.org/book/C1-C_intro/structs.html#FigStructStack) illustrates the contents of the call stack just before the `checkID` function returns.
