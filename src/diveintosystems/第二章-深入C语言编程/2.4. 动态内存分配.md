...
> [!NOTE]
> Heap Memory Management, malloc and free C 标准库实现了 malloc 和 free，它们是其堆内存管理器的编程接口。调用时，malloc 需要在未分配的堆内存空间中找到一块可以满足请求大小的连续块。堆内存管理器维护堆内存的未分配扩展区的空闲列表，其中每个扩展区指定连续的未分配堆空间块的起始地址和大小。
